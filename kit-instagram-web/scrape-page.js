...
const HANDLE = process.argv[2] || 'mattperruquers';
...
(async()=>{
  ...
  const ctx = await chromium.launchPersistentContext(TMP, { channel:'chrome', headless:true, viewport:{width:1280,height:1400} });
  const page = ctx.pages()[0] || await ctx.newPage();
  const out = { handle:HANDLE };
  // capture profile json from network if it loads
  let netUser = null;
  ...
  try{
    ...
    const meta = await page.evaluate(()=>{
      ...
      const t = (n)=>{ const el=document.querySelector('meta[name="'+n+'"]'); return el?el.content:null; };
      return { ogTitle:g('og:title'), ogDesc:g('og:description'), ogImage:g('og:image'), desc:t('description'), title:document.title };
    });
    out.meta = meta;
    ...
    const imgs = await page.evaluate(()=>{
      ...
      document.querySelectorAll('main img, article img').forEach(im=>{
        ...
        if(s && s.includes('fbcdn') && w>=200) arr.push({src:s, alt:alt.slice(0,200), w});
      });
      return arr;
    });
    // dedupe
    ...
    // download profile pic from ogImage
    if(meta.ogImage){ out.profileSaved = await dl(meta.ogImage, path.join(OUT,'profile-mat.jpg')); }

    // download grid posts (skip first if it's avatar-sized)
    out.saved=[]; let n=0;
    for(const im of out.allImages){
      n++; if(n>12) break;
      const ok = await dl(im.src, path.join(OUT,'post-'+n+'.jpg'));
      if(ok) out.saved.push({file:'post-'+n+'.jpg', alt:im.alt});
    }
    out.downloaded = out.saved.length;
  }catch(e){ out.error=String(e.message).slice(0,300); }
  fs.writeFileSync(path.join(__dirname,'ig-data-mat.json'), JSON.stringify(out,null,2));
  console.log(JSON.stringify({handle:out.handle, meta:out.meta, downloaded:out.downloaded, followers:out.followers, fullName:out.fullName, error:out.error, imgs:(out.allImages||[]).length},null,2));
  await ctx.close();
  fs.rmSync(TMP,{recursive:true,force:true});
})();
